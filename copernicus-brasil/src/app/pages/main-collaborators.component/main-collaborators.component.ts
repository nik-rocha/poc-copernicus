...
export class MainCollaboratorsComponent implements OnInit {
  ...
  currentUser = computed(() => {
    ...
    try {
      ...
      const message =
        e.response?.data?.message || 'Erro ao decodificar o token para conseguir o usuário atual.';
  ...
  collaborators = computed(() => {
    const targetOrg = (
      this.isManager() ? this.selectedOrganization() : this.userOrganization()
    ) as any;
  ...
  filteredCollaborators = computed(() => {
    ...
    return this.collaborators().filter(
      (c: any) => c.fullName?.toLowerCase().includes(term) || c.email?.toLowerCase().includes(term),
    );
  });
  ...
  filteredDevices = computed(() => {
    ...
    return this.devices().filter(
      (d: any) => d.model?.toLowerCase().includes(term) || d.assetTag?.toLowerCase().includes(term),
    );
  });
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      const orgsData = Array.isArray(orgsResponse)
        ? orgsResponse
        : orgsResponse
          ? [orgsResponse]
          : [];
  ...
  openModal(entity: ModalEntity, action: ModalAction, target?: any): void {
    ...
    if (action === 'edit' && target) {
      ...
      if (entity === 'organization') {
        this.organizationForm = {
          ...target,
          registrationCode: this.formatCnpj(target.registrationCode || ''),
        };
      }
    }
    ...
    if (action === 'create') {
      const targetOrgId =
        this.selectedOrganization()?.idOrganization ?? this.userOrganization()?.idOrganization;
      ...
      if (entity === 'device')
        this.deviceForm = { model: '', assetTag: '', organizationId: targetOrgId };
      if (entity === 'organization')
        this.organizationForm = { corporateName: '', registrationCode: '' };
    }
  ...
  async confirmAction(): Promise<void> {
    ...
    if (action === 'create' || action === 'edit') {
      if (entity === 'collaborator') {
        const emailError = this.validateEmail(this.collaboratorForm.email);
        if (emailError) {
        ...
        if (action === 'create') {
          const passwordError = this.validatePassword(this.collaboratorForm.password);
          if (passwordError) {
      ...
      if (entity === 'organization') {
        if (!this.organizationForm.corporateName) {
          this.errorMessage.set('Preencha o nome da organização.');
          return;
        }

        const cnpjError = this.validateCnpj(this.organizationForm.registrationCode);
        if (cnpjError) {
    ...
    const targetId =
      (this.modalTarget() as any)?.idCollaborator ??
      ...
      if (entity === 'collaborator') {
        ...
        if (action === 'edit') {
          ...
          if (isSelf && emailChanged) {
            alert(
              'Como você mudou seu e-mail, logue no sistema novamente para confirmar a mudança.',
            );
            this.logout();
            return;
          }
  ...
  private scrollToItem(entity: ModalEntity, id: number): void {
    const prefix =
      entity === 'collaborator' ? 'collaborator' : entity === 'device' ? 'device' : 'organization';
  ...
  onCnpjInput(value: string): void {
    this.organizationForm.registrationCode = this.formatCnpj(value);
    this.errorMessage.set('');
  }

  private validateEmail(email: string | undefined): string | null {
    if (!email) return 'Preencha o e-mail.';
    if (email !== email.toLowerCase()) return 'O e-mail não pode conter letras maiúsculas.';
    if (!/^[a-z0-9._%+-]+@[a-z0-9.-]+\.com$/.test(email))
      return 'O e-mail deve ser válido e terminar em .com.';
    return null;
  }

  private validatePassword(password: string | undefined): string | null {
  ...
  private validateCnpj(cnpj: string | undefined): string | null {
    if (!cnpj || !/^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/.test(cnpj)) {
      return 'Preencha o CNPJ da organização (xx.xxx.xxx/xxxx-xx, apenas números).';
    }
    return null;
  }
}
